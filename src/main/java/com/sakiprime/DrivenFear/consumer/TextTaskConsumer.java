...
public class TextTaskConsumer {
    ...
    public void taskConsumer(
        ...
        try {
            ...
            if (!isSuccess) {
                channel.basicAck(deliveryTag, false);
                log.warn("文本任务扣款存单失败，已转存到人工队列。订单号: {}", request.getOrderId());
                return;
            }

            Result<Void> handleResult = taskConsumerService.sendTaskToApi(request.getOrderId());
            if (handleResult.getCode() !=200){
                log.warn("文本任务处理失败，重新入队。订单号: {}", request.getOrderId());
                throw new Exception(handleResult.getMsg());
            }
            channel.basicAck(deliveryTag, false);

            log.info("文本任务处理成功，已确认消息。订单号: {}", request.getOrderId());
            ...
            try {
                ...
                if(correlation.getConsumerRetryCount()<3){
                    ...
                    rabbitTemplate.convertAndSend(correlation.getQueue(),
                         correlation);
                }
                else{
